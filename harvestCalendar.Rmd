---
title: "harvestCalendar"
author: "Xiaofei Zhang"
date:  "`r format(Sys.time(), '%B %Y')`"
output: html_document
---

### __1. think about a new way to manage harvest calendar__
...
__the first step is to generate the rough calendar based on the month after planting__

__trials downloaded from CassavaBase__
...
print("trials without planting date")
meta %>%
...
__Camilo's planning___
__trials from Camilo's file; compare and find the extra trials__
...
print("trials only in Camilo's file")
setdiff(camilo_plan$studyName, camilo_sel$studyName)
print("trials only in cassavaBase")
...
__assign the harvesting in single days__
...
same_harvest = camilo_long %>%
  ...
  dplyr::filter(n > 1L) 
print("trials or harvests with duplications")
same_harvest
nrow(same_harvest) # if not 0, there are duplicates date+plantday
```


__generate the first version of calendar to start__
```{r}
...
__print out the first version of harvest calendar__
...
### __2. modify the harvest calendar based on Camilo's plan__

__manually move the harvesting time of trials in excel__
...
__Here is the planting calendar__
